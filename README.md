...
**Cuek** is a minimal, TypeScript-first UI library for building reactive browser interfaces with a small runtime and predictable architecture.
...
Cuek targets developers who want component-based UI without framework overhead.
...
Cuek builds minimal Virtual DOM templates strictly to power local, fast DOM patching.

> **Note:** Cuek may migrate entirely to an Ahead-Of-Time (AOT) compiler architecture in the future. This would completely eliminate the Virtual DOM and DOM-patching engines, allowing for true direct-to-DOM updates like Svelte or SolidJS, while preserving our explicit component execution model.
...
Cuek is designed around strict typing:
...
Cuek prioritizes control over automation.
...
Cuek executes the function right after the component paints that node cleanly to the DOM:
...
Cuek provides three primary APIs for rendering and updating the DOM explicitly.
...
Cuek does not use an automated Virtual DOM scheduler. Instead, reactivity is explicit and localized. You precisely dictate when updates generate patching routines by using `rerender`:
...
3. **Rerender by Component Context**: Renders solely the specific component instance issuing the call. To ensure zero-magic predictability, Cuek passes a reliable execution context as the second argument to any component.
...
To prevent memory leaks when components are removed dynamically, Cuek provides an explicit `unmount` API to clean up active trees and unbind listeners.

```ts
import { unmount } from "cuek";
...
While Cuek eliminates complex implicit lifecycle scheduling, you will still need to perform side-effects predictably (like data fetching or WebGL rendering) when working with direct DOM mutations.

Because component functions are executed only once to initialize state, your `ctx.onMount` logic is inherently immune to infinite-loop fetch bugs. Cuek saves and executes the inner closure returned by your component on every `ctx.rerender`:
