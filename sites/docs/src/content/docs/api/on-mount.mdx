...
Unlike hooks in other libraries, `onMount()` does not re-run when your props change. In Rin, your component function only runs once to set things up. This means `onMount()` is truly a "once per lifetime" event for the component instance. If you need to respond to data changes, handle that in your event listeners or via `rerender()`.
